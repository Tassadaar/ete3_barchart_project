...
class Taxon:
    ...
    def __init__(self, name, seq):
        ...
        self.abs_freq_dict = {aa: self.seq.count(aa) / len(self.seq) for aa in self.all_amino_acids}
        self.display_freqs = {}
        self.group1_freq = {}
        self.group2_freq = {}
        self.other_freq = {}
        self.group1_rel_freq = {}
        self.group2_rel_freq = {}
        self.other_rel_freq = {}

    def get_all_relative_freq(self, avg_freq_dict):
        return {aa: self.abs_freq_dict[aa] - avg_freq for aa, avg_freq in avg_freq_dict.items()}

    def set_subset_abs_freq(self, subsets):
        for key, value in self.abs_freq_dict.items():
            if key in subsets[0]:
    ...
    def set_subset_rel_freq(self, subsets, avg_freq_dict):
        for aa, avg_freq in avg_freq_dict.items():
            if aa in subsets[0]:
                # group1_rel_freq[aa] = self.group1_freq[aa] - avg_freq
                self.group1_rel_freq[aa] = self.group1_freq[aa] - avg_freq
            elif aa in subsets[1]:
                # group2_rel_freq[aa] = self.group2_freq[aa] - avg_freq
                self.group2_rel_freq[aa] = self.group2_freq[aa] - avg_freq
            else:
                # other_rel_freq[aa] = self.other_freq[aa] - avg_freq
                self.other_rel_freq[aa] = self.other_freq[aa] - avg_freq

    def get_subset_rel_freq(self):
        return [self.group1_rel_freq, self.group2_rel_freq, self.other_rel_freq]

    def set_display_freqs(self, mode, avg_freq_dict=None):
        if mode == 'absolute':
            self.display_freqs = self.abs_freq_dict
        elif mode == 'relative':
            self.display_freqs = self.get_all_relative_freq(avg_freq_dict)
    ...
    def calculate_chi_square(self, align_freqs):
        ...
        for aa in self.all_amino_acids:
            ...
            observed_count = self.abs_freq_dict[aa] * taxon_seq_len
